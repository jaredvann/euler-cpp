...
int solveTriangle(int g[15][15], int s) {
    if (s == 1) {
        return g[0][0];
    } else {
        int g1[15][15], g2[15][15] = {0};
        ...
        return g[0][0] + max(solveTriangle(g1, s-1), solveTriangle(g2, s-1));
    }
}
...
int main(int argc, char const *argv[]) {
    ...
    for (int i = 0; i < 15; i++) {
        for (int j = 0; j <= i; j++) {
            g[i][j] = ((int)s[(l*3)+(j*3)]-48)*10 + ((int)s[(l*3)+(j*3)+1]-48);
        }
        l += i+1;
    }

    cout << solveTriangle(g, 15) << endl;
    return 0;
}
